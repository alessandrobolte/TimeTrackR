...
function createAdminView(user) {
  ...
  <header class="main-header">
    ...
    <div class="flex-gap"><button id="logoutBtn">Abmelden</button></div>
  </header>
  <main class="content">
    <section class="card">
...
function stopLocalTimer() {
  ...
  const session = (cat.sessions || [])
    .slice()
    .reverse()
    .find(s => !s.end && (s.start === active.start || s.start <= active.start));
...
function openManualModal() {
  ...
  modal.querySelector('#madd').onclick = () => {
    ...
    if (!catId || !date || (hours === 0 && mins === 0)) {
      alert('Bitte Kategorie, Datum und Dauer angeben.');
      return;
    }
...
async function exportCsv() {
  try {
    ...
    a.href = url;
    a.download = 'zeitmanagement_export.csv';
...
function renderSessions() {
  ...
  tbody.querySelectorAll('td[contenteditable]').forEach(td =>
    td.addEventListener('blur', e => {
      ...
      if (!cat) return;
      const sess = (cat.sessions || []).find(s => s.id === sid);
      if (!sess) return;
      sess.note = td.textContent.trim();
      saveClientData();
    })
  );
}
...
function renderAdminList(data) {
  const div = document.getElementById('adminLemList');
  if (!div) return;

  const rows = data.map(u => {
    const sum = u.categories?.reduce((acc, c) => {
      const mins = (c.sessions || []).reduce((m, s) => m + (s.durationMin || 0), 0);
      return acc + mins;
    }, 0) || 0;

    return `<div class="admin-row">
      <strong>${escapeHtml(u.displayName)}</strong>
      <span>${minutesToHuman(sum)}</span>
    </div>`;
  });
...
async function adminExport() {
  try {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = 'admin_gesamt_export.csv';
    document.body.appendChild(a);
    a.click();
    a.remove();
    URL.revokeObjectURL(url);
...
(async () => {
  try {
    ...
    console.warn('Kein aktiver Login:', e);
  }
})();
